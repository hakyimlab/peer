...
cWNode::cWNode(PMatrix E1){
	...
	this->Xprec = E1.rows()*E1.transpose()*E1; // very approximate
}
cXNode::cXNode(){}
cXNode::cXNode(PMatrix E1){
	this->E1 = E1;
	this->E2S = E1.cols()*E1.transpose()*E1;
...
cXNode::cXNode(PMatrix E1, PMatrix prior_offset, PMatrix prior_prec){
	this->E1 = E1;
	this->E2S = E1.rows()*E1.transpose()*E1;
...
//		covs = temp;
	...
	if (Nc>0)
	{
		...
		//cout << covs << endl << covs_original << endl;
	}
	else {
		residuals = pheno.E1;
	}
	...
	Xprec_prior.diagonal().block(0,0,Nc,1) = PMatrix::Ones(Nc,1)*covariate_prec; // covariates are very precise
